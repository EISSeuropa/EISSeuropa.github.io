...
- **Co-authors now actually appear on the programme grid, presenter first.** The co-author work shipped earlier read `primaryauthors` / `coauthors`, but the Indico **timetable** export (the endpoint the grid is built from) carries a single combined `authors` array plus a `presenters` subset, so those keys were always empty and only the presenter showed. `sync-indico.py` now reads `authors` (with the `primaryauthors` + `coauthors` split kept as a fallback for other endpoints) and resolves names from `firstName` / `familyName` as well as `name`. The author list is ordered **presenter(s) first** (each marked with the microphone) followed by the non-presenting authors and co-authors. The regenerated `indico.json` surfaces 17 previously-missing co-authors across 20 multi-author papers.
