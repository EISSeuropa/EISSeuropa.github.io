...
- **Photo gallery on `/2025` (Thessaloniki).** Nine photographs from the 8th Annual Conference, added as a "Conference gallery" section between the keynote / round-table block and the venue card. Source images (supplied by the maintainer) were web-optimised to 1500 px / quality 60 (~180–310 KB each) and live under `src/assets/images/2025/`. The high-quality conference film is too large to commit to the repo and is not included; it needs external hosting (YouTube), after which the page's existing gated recordings section can be flipped on.
