...
- **ESSC host-city map projection re-tuned to look familiar** (EN, FR, DE). v2.23.0 applied a full cos(50°) correction (y-scale of 1.82×) to compensate for the Plate-Carrée projection's horizontal stretch. The result rendered Italy and Norway absurdly tall on desktop. Re-tuned to a 1.4× y-scale, which sits between the over-corrected 1.82× and the under-corrected 1.0× (no correction). ViewBox moves from 1000×1274 to 1000×980 (≈ 1.02 W/H, essentially square), `.essc-map-figure` `max-width` from 40rem to 44rem so the rendered map lands at ~704 × 690 px on desktop. All seven dot/label coordinates pre-multiplied by 1.4 in all three locale files. The factor was tuned by eye against typical textbook reference maps for this lat[35,71] / lon[-12,32] slice of Europe.
