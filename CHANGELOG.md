...
- **Programme-grid copy reworded on `/YYYY` (EN + FR + DE).** The live-sync lead now reads "This programme is synchronised on a daily basis from Indico. Changes may appear with a slight delay." (was a longer "~24 hours" phrasing). The footnote drops the developer term "source of truth" (which reads oddly to a public audience) in favour of "Indico holds the authoritative programme. This grid is a daily snapshot." FR and DE updated to match by hand; both strings also lose their em-dashes.
