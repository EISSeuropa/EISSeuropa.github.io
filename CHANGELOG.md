...
- **`/initiative` NetSec section now shows EISS people in NetSec leadership roles (EN + FR + DE).** The block previously listed the three EISS officers under their EISS titles. It now presents the EISS board and team members who hold a NetSec leadership role, each labelled with that NetSec role and their EISS affiliation, to make the dual-institutional overlap explicit (almost the entire NetSec leadership sits on the EISS board). Roles live in `src/_data/netsec.js` (sourced from netsec-cost.eu/about.html); `boardSorted.js` joins them to board headshots by an accent- and apostrophe-folded name key and exposes `netsecLeaders`. New shared partial `src/_includes/netsec-leaders.njk` renders the strip across all three locales, with the framing and the dual-affiliation tags localised via a new `netsec` i18n block. The NetSec role titles themselves stay in English (the Action's own working language).
