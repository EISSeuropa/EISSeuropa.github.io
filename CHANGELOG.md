...
- **The hero image now ships a phone-sized variant.** The full-bleed hero on the homepage and `/2026` (EN + FR + DE) is the same 1600×900 JPG, served at full resolution to every device including phones. It now carries a `srcset` with an 800-wide variant (127 KB versus 275 KB) and `sizes="100vw"`, so a phone downloads roughly half the bytes for its largest, highest-priority image while desktop still gets the full-resolution file. A first, highest-value slice of the responsive-images work; the galleries and the rest of the rollout (and a build-time pipeline for it) stay on [#554](https://github.com/EISSeuropa/EISSeuropa.github.io/issues/554).
