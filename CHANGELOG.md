...
- **Institutional logos on the `/2026` joint-organisers strip.** All three orgs now ship with a logo alongside the institution name: the EISS network mark (reused from `src/assets/images/brand/logo-mark.svg`), the NetSec mark (carried over from the sister `netsec.github.io` repo), and the official Stockholm University landscape wordmark (provided by the maintainer from the SU brand pack, RGB-HEX / EN). New `src/assets/images/partners/` directory holds the three files. CSS bumped logo height from 28 px → 32 px so the SU wordmark stays legible, and added a soft off-white tile in dark mode behind every logo so the navy SU mark (`#002F5F`) and the dark NetSec mark stay visible against the dark `--bg-elev` panel. The EISS network mark (bright `#73caff`) sits happily on the same tile.
