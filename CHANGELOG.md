...
- **Board cards now show each member's recent publications, drawn from ORCID.** Open a person's detailed card on the People page and, where they list an ORCID iD, their most recent works appear under a "Recent publications" heading, each title linking to its DOI. The data loads lazily: the page ships an empty placeholder and fetches the works once, only when a card scrolls into view, so the People page stays light at first paint. The heading is hand-translated (EN, FR, DE), and a member without an iD shows nothing. The works come from the same weekly ORCID refresh that powers the members section on `/outputs`, so coverage grows as members add their iD to the directory form. Closes [#719](https://github.com/EISSeuropa/EISSeuropa.github.io/issues/719).
- **A Publications & outputs page at `/outputs` (EN + FR + DE).** Two distinct sections: the network's own publications (the book series, the Prix Bastien Irondelle, briefs and survey results — curated, expanding) and, separately, "Recent publications by our members" — recent works our board and community members have published in their own right, drawn live from their public ORCID records and refreshed weekly. A new `scripts/sync-orcid.py` (a step on the board-sync workflow) fetches each member's public ORCID works into `src/_data/orcidWorks.json`; the page renders them with DOI links, each author linked to their People-page entry. Members who add their ORCID iD to the directory form appear automatically. Linked from the visual sitemap. First slice of [#718](https://github.com/EISSeuropa/EISSeuropa.github.io/issues/718); the curated network-publications catalogue continues under [#97](https://github.com/EISSeuropa/EISSeuropa.github.io/issues/97).
