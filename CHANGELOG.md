...
- **New "Co-Director" leadership role on the People page.** The board-sync pipeline and the page now recognise a **Co-Director** role, placed in the Leadership section just below Founding Director (`scripts/board-source.json` roles table, tier 2; `boardSorted.js` derives the section from it). This clears the sync warning where the Form's "Co-Director" value fell back to "Board Member", and Moritz Weiss moves from Secretary-General to Co-Director on the next board sync.
