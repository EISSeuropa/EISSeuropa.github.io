...
- **"Speaking or chairing?" note and inline livestream signposting on `/2026` (EN + FR + DE), adapted from the NetSec sister site.** A new `speaking-chairing.njk` adds a discreet soft banner (a mic icon and a subtle rounded fill, in the spirit of the NetSec sister page) just before the programme, pointing speakers and chairs to the shared conference FAQ on the NetSec site (which carries the per-role links, edit My Session for chairs and My Contributions for speakers, plus visa-letter guidance) and to the official printable programme PDF. The FAQ URL is a per-conference `faqUrl` field in `conferences.js`. The standalone "Livestreamed sessions" block was removed (it distracted from the programme): in its place the grid carries a compact signpost above it ("the plenary sessions are livestreamed, tagged below", with a "Join online on Indico" link) and a **"Livestream" pill** on each livestreamed session. `sync-indico.py` flags the livestreamed programme slots by matching the Type-aware `livestreamed` list, so the pill catches every plenary and roundtable (including the one whose sessionCode isn't a plain `RT`). The card and signpost are hidden in print; the data-only `livestreamed` list is retained.
- **Co-authors on the live programme grid, with the presenter marked.** Indico distinguishes who presents a paper (`speakers`) from the fuller author list (`primaryauthors` + `coauthors`), but the grid previously showed only the presenters, so co-authors were missing against the printed programme. `scripts/sync-indico.py` now captures the full author list per contribution into an `authors` array, each carrying an `isSpeaker` flag. The `/YYYY` programme grid (EN + FR + DE) renders every author and picks out the presenter(s) with a small microphone and bolder text, while co-authors stay in the muted body colour. A screen-reader-only "Presenter:" label accompanies the icon. Co-authors that sit behind Indico's authenticated API populate on the next authenticated sync run. Falls back to the old speakers-only rendering when a contribution carries no separate author list.
