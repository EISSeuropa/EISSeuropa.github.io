...
- **The Indico sync now opens a reviewable PR detailing exactly what changed.** `sync-indico.py` compares the previous `indico.json` against the freshly-built data (ignoring the always-moving `syncedAt` timestamp) and produces a Markdown summary: which sessions, papers, speakers, co-authors, times, rooms, and abstracts were added, removed, or changed. When the difference is substantive, `.github/workflows/sync-indico.yml` opens a PR on `indico-sync/auto` with that summary as the body, so the maintainer sees precisely what a sync will alter before it goes live. When only the timestamp moved, it direct-commits a quiet refresh as before (no empty PR). Previously every sync direct-committed with a bare "sync events" message and no detail.
