...
- **ESSC host-city map on `/initiative`** — inline SVG showing 36 European countries projected from [Natural Earth](https://github.com/nvkelso/natural-earth-vector) 110m admin-0 vectors (CC0). Path data lives in a new `src/_includes/europe-outline.njk` partial (~18 KB) so the geometry isn't duplicated across the three locale files; included verbatim from EN / FR / DE. Each host city renders as an accented dot inside a `<g>` linked to the per-edition page, with `<text>` labels above the dot (city) + below (year). 2017 Paris cluster is a single dot covering 2017/2018/2019 per the design call; 2020 online edition is acknowledged in the figcaption rather than placed on the map; 2026 Stockholm uses a "next" accent variant to flag it as upcoming. Wrapped in a glass-card `<figure>` consistent with the rest of the page. Closes #131. (#131, #132)
