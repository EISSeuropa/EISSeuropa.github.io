...
- **The homepage conference card's status badge is legible in light mode.** The registration / "Happening now" badge on the featured card was styled for the dark `/YYYY` hero (white text on a translucent-white chip), so on the light homepage card the label and its call-to-action button rendered white-on-white and effectively vanished, leaving a lone status dot. The badge now uses surface-aware tokens when it sits on the featured card, so the label, the button and the chip stay legible in both light and dark themes. From conference-week QA.
