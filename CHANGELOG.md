...
- Programme room badges no longer pick two different visual languages depending on whether the room matches the conference default. Every slot's room renders as a pill now — neutral grey for default-room slots, accent blue for off-default (parallel panels in a different hall, breaks on a different floor). The pill shape unifies the grid visually while colour still distinguishes the exceptions. Indico is now the single source of truth for the room string: the sync passes the field through verbatim (the operator added `D House, ` to all locations on Indico, so no normalisation is needed). Removed the `_canonical_room_display` helper.
- Replaced the default-vs-elsewhere binary with **a stable colour-per-location palette**. The sync now counts how often each distinct room is used across the programme and assigns a 0-based colour index in frequency-descending order (alphabetical tie-break). For ESSC 2026: Lecture Hall 8 (most used) = blue, Lecture Hall 9 = purple, Floor 3 = amber. The template picks one of six hues via the `.programme-slot-room--c{n}` modifier; a seventh-room fallback renders as neutral grey. Dark-mode palette lifts the saturations and lightnesses so pills stay legible. Parallel-row column sort uses the same index (most-frequent room always left-most). Removed the default-room hint and the `programme.defaultRoomLine` i18n key.

### Added

- Bio teaser + Read-more expander on every board card. The first ~180 chars (truncated at the word boundary) render inline; the full bio sits inside a native `<details>` element so opening it is keyboard-accessible and SEO-friendly without JS. Arthur and John's old support-team bios are now lifted back into the board cards via this mechanism; Eugenio's Support Staff card also gets it.
- Functional Responsibility now renders as a quiet **pill** alongside the primary role (frosted background, 999px radius, 1px neutral border, sentence case) — same shape language as the programme room pills, but a neutral hue so it doesn't fight the accent-coloured role.
- Optional **social-link / ORCID / website / public-email icon row** at the bottom of each card. Gated on field presence — invisible today (no entries have these yet) and will light up automatically when the Form starts populating them. Added Lucide `globe` icon + ORCID, Bluesky, and Mastodon brand glyphs to `src/_includes/icons.njk`.
- Optional **country line** under the affiliation, rendered with a tiny map-pin icon. Gated on field presence.
- Updated `scripts/board-source.json` and `scripts/sync-board.py` to match the live Form's column shape: 18 questions (renamed several, added Country, Public email, Functional responsibility, Working-group involvement, website / ORCID / LinkedIn / Twitter / Bluesky / Mastodon). The roles table collapsed to five entries — *Founding Director / Treasurer / Secretary-General / Board Member / Support Staff* — with the old coordinator titles moved into a separate `functional_responsibilities` list. `sync-board.py` joins Position + Institution into the displayed `affiliation`, keeps Country as its own field, and nests social-link URLs under `links: {}`.
- Eugenio's `board.json` entry refactored to the new model: `role: "Support Staff"`, `functionalResponsibility: "Communications Coordinator"`. Sectioning still puts him in Support Staff per the `kind: support` mapping.
- Small "On the EISS team? Update your bio →" link at the bottom of `/board`, pointing to the live Google Form. Quiet typography — muted text, dotted underline, accent only on hover — so it nudges Board members toward self-service without reading as a CTA. URL sourced from the `form_url` already in `scripts/board-source.json` so it stays in sync with the pipeline config. Localised in EN / FR / DE.
- **Identity-key matching in `scripts/sync-board.py`** — a first-token-plus-last-token identity key replaces the previous strict slug as the primary dedup mechanism. Collapses middle-initial drift (`Arthur PB Laudrain` ↔ `Arthur Laudrain`), case-insensitive honorific changes (`Dr` ↔ `Prof` ↔ `Lt Gen`), and the case where the same person submits from a different Google account. The honorific regex covers `Dr / Prof / Pr / Mr / Ms / Mrs / Mx / Lic / Lt Gen / Lieutenant General / General / Colonel / Admiral`, double honorifics, and now matches case-insensitively. When a near-match collapses two submissions or matches a submission to a prior entry with a different name, the script logs a loud "~ collapsed" warning so the PR reviewer can sanity-check. Verified against 16 drift cases including all 6 that previously failed.
- `/board` modernised — three sections (Leadership, Board Members, Support Staff) split from the previous flat 19-card grid using the existing `tier` field on `scripts/board-source.json`. Same card size across all sections; visual hierarchy comes from the section headings, not card sizing. Each section's cards sort by tier then surname. The new `src/_data/boardSorted.js` is a pure-derived view: `board.json` (the Form-pipeline contract) and `scripts/board-source.json` stay untouched.
- ESSC-speaker mic icon on board cards. A small amber 🎤 appears next to the role on any card whose person is chairing, discussing, or speaking at the live ESSC edition. Matched at build time between `board.json` and `indico.json` programme data via a "first-token + last-token" identity key (handles "Dr Arthur PB Laudrain" matching "Arthur Laudrain"). Tooltip "Chairing or speaking at the current ESSC" in EN / FR / DE.
- `functionalResponsibility` field on board members. Renders as a secondary label next to the primary role (e.g. `BOARD MEMBER · Technology Coordinator`). Quieter weight, sentence case, muted colour — visually signalled as secondary to the role. Used for Board Members who also hold a coordinator function.

### Changed

- Arthur and John moved from the support team to the Board, keeping their Technology / Events Coordinator titles as `functionalResponsibility`. Eugenio remains in Support Staff as Communications Coordinator.
- The third board section renamed: `Support Team` → `Support Staff` in EN; `Équipe de soutien` → `Personnel de soutien` in FR; `Unterstützungsteam` → `Support-Personal` in DE.
