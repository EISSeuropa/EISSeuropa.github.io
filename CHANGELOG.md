...
- **`/policy.html` §5 documents the YouTube embed.** New list item under *Third-party services* describes the two-stage lazy-load posture, names the domains reached at each stage (`i.ytimg.com` before click, `youtube-nocookie.com` after), and links Google's privacy policy. FR + DE policy translations marked stale for the maintainer to refresh by hand (per the no-machine-translation rule).
