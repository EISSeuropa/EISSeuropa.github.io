...
- **Lazy YouTube embed partial.** New `src/_includes/youtube-embed.njk` renders a poster image plus play button as a static placeholder, mounts the privacy-enhanced `youtube-nocookie.com` iframe only when the visitor clicks. Wired into `/2025` behind a `{% if false %}` gate awaiting the real playlist ID from the EISS YouTube channel (maintainer flips the gate when the ID is in). Two-stage privacy posture: only the poster thumbnail is requested from `i.ytimg.com` before click (no cookies, no YouTube JavaScript, no ~500 KB of YouTube CSS), and the iframe loads from the cookieless variant. Honours `prefers-reduced-motion` via the existing reveal-animation gate. Closes the foundation work for #197, ready for rollout to remaining archive years (`/2019` through `/2024`) once playlist IDs are confirmed.
