...
- **A Publications & outputs page at `/outputs` (EN + FR + DE).** Two distinct sections: the network's own publications (the book series, the Prix Bastien Irondelle, briefs and survey results — curated, expanding) and, separately, "Recent publications by our members" — recent works our board and community members have published in their own right, drawn live from their public ORCID records and refreshed weekly. A new `scripts/sync-orcid.py`, run weekly by a dedicated workflow, fetches each member's public ORCID works into `src/_data/orcidWorks.json`; the page renders them with DOI links, each author linked to their People-page entry. Members who add their ORCID iD to the directory form appear automatically. Linked from the visual sitemap. First slice of [#718](https://github.com/EISSeuropa/EISSeuropa.github.io/issues/718); the curated network-publications catalogue continues under [#97](https://github.com/EISSeuropa/EISSeuropa.github.io/issues/97).
...
- **A speaker index at `/speakers.html`.** A searchable, surname-ordered list of everyone who has presented at a European Security Studies Conference, assembled from the programmes of every edition: 492 speakers across 12 conferences, with the papers each gave (and the year) expandable inline and linking back to that conference page. Each entry shows the person's most recent affiliation on record (not a cumulative list), and a name particle like "van" files under the right letter ("van Hooft" under H). Each entry is tagged with broad themes (the nine permanent EISS panel sections plus recurring themes derived from the open panels, e.g. Deterrence, Intelligence, European and transatlantic security), shown as chips and filterable from a dropdown; themes are inferred from each paper's panel and left untagged where a session title is too generic to be confident. The list can also be narrowed by a find-by-name box (diacritic-insensitive), the chosen theme is mirrored in the URL so a filtered view is shareable and survives Back, a Clear control resets both, and the result count is announced to assistive technology. All of it is progressive enhancement: with JavaScript off the full list still renders. Speakers who are EISS board or community members are badged and linked to their profile. Built on a new `src/_data/corpus.js` data spine that flattens the archive programmes plus the live Indico data into one paper/speaker dataset (the same spine the planned paper index and open-data export will reuse). Names are deduplicated conservatively — normalising case, accents and leading honorifics, but biased to leave near-duplicates rather than risk merging two different people, with a manual alias hatch for known cases. The corpus totals (speakers, conferences, papers) head the page as summary cards, and the index is signposted from the visual sitemap, every conference programme page, and the People page by a small link-card that points to the reader's own language. Each entry shows the years that person presented as pills, with a headshot for EISS members, and the filter bar is a faceted panel that narrows the list by name, by theme or by a specific edition. Available in English, French and German: the interface is hand-translated, while speaker names, paper titles, affiliations and theme labels stay in their original language. Closes [#635](https://github.com/EISSeuropa/EISSeuropa.github.io/issues/635).
...
- **A cross-year paper index at `/papers` (EN + FR + DE).** Every paper presented at a European Security Studies Conference since 2017, in one browsable, searchable place: 510 papers across the editions, each with its authors, affiliations, year and panel, linking back to that year's programme. The list can be filtered by year and by theme, or searched across titles, authors and affiliations, with the chosen filters mirrored in the URL so a view is shareable. It is server-rendered, so the site search indexes every title and the full list still renders with JavaScript off. Built on the same `corpus.js` data spine as the speaker index. The theme facet is derived from each paper's panel. Closes [#632](https://github.com/EISSeuropa/EISSeuropa.github.io/issues/632).
- **Board cards open a full profile, with each member's recent publications.** The People page cards stay compact and uniform (name, role, affiliation, research themes, contact links), and a "View profile" control on the cards that have more to show opens a dialog with the member's full biography and, where they list an ORCID iD, their recent publications linking to each DOI. Keeping the detail in a dialog means an open profile no longer stretches the cards beside it, the layout problem that came from expanding long biographies in place. The dialog is a native element, so focus handling and Escape-to-close come for free, the publications are rendered at build time from the same weekly ORCID refresh that powers `/outputs`, and with JavaScript off the control is a plain link that reveals the profile inline. Closes [#719](https://github.com/EISSeuropa/EISSeuropa.github.io/issues/719).
...
- **The speaker and paper indexes are now one "Conference Navigator".** They were two pages over the same conference corpus, the people view and the paper view of one dataset. They are now a single archive with a "By person / By paper" toggle: browse everyone who has presented since 2017, or every paper they gave, and filter by year or theme within each. Both `/speakers` and `/papers` stay live (each opens the Navigator on its own view, and the chosen view rides in the URL so it is shareable), so no existing link breaks. First phase of the people-and-outputs consolidation in [#756](https://github.com/EISSeuropa/EISSeuropa.github.io/issues/756). EN + FR + DE.
- **The Initiative's leadership is updated on the People page.** Moritz Weiss and Chiara Ruffa are now Co-Directors and lead the board, Hugo Meijer (who founded the Initiative) becomes Honorary Director, and the Secretary-General role is retired for now. Closes [#344](https://github.com/EISSeuropa/EISSeuropa.github.io/issues/344) and [#563](https://github.com/EISSeuropa/EISSeuropa.github.io/issues/563).
- **Publications and outputs fold into the Initiative page.** The standalone `/outputs` page is retired: the network's own publications now sit in a "Publications & outputs" section on `/initiative`, and members' personal works live on their profile pages rather than a shared list. `/outputs` (EN + FR + DE) redirects to `/initiative#outputs`, so existing links keep working. Part of the people-and-outputs consolidation in [#756](https://github.com/EISSeuropa/EISSeuropa.github.io/issues/756).
