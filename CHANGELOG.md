...
- **Paper pages can now link the version later published in a journal.** Where an ESSC paper was subsequently published, its `/papers/<slug>` page links the published article and shows its DOI, turning the archive into a "presented at ESSC → published here" record. Confirmed matches live in `src/_data/paperLinks.json`; a paper with a confirmed link gets a page even when it has no abstract. The first such link is the 2018 nuclear-alliances paper. (The matches are proposed by a new ORCID-anchored matcher and recorded only after human review, per [#805](https://github.com/EISSeuropa/EISSeuropa.github.io/issues/805).)
