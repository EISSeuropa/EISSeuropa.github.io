...
- **Recovered 2021 Lisbon conference programme on `/2021`.** The page previously had a broken "programme" card referencing a file with no link and an "open in Incognito" workaround note. The full two-day session programme has been reconstructed from the original programme images (`eiss21-1.png` / `eiss21-2.png`) into structured HTML: time-stamped slots, parallel panels with their chairs, both keynotes (Anne Deighton's *Security Studies 2020: Blindsided by Brexit?* and the Portuguese Minister of Defence), and the closing multidisciplinary roundtable. New reusable `.programme-archive` CSS component (time column plus session bodies, parallel panels side by side, collapses on narrow viewports). Sessions are labelled "Day one / Day two" rather than calendar dates, because the source images and the page header disagree on the September dates (tracked in #230).
