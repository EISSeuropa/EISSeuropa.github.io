...
#    - translate visible prose by hand (no machine translation)
...
- **Substantive edit**: re-translate `index.fr.njk` and `index.de.njk` by hand to match, then:
  ```
...
- **No machine translation, ever** (CLAUDE.md §1). Every FR and DE string is translated by hand.
- **Chrome strings** (nav, footer, buttons): translated by hand and kept short.
- **Page prose** (the visible content of each `.fr` / `.de` page): translated by hand. Mark as `beta`.
- **Legal pages** (privacy, T&C, accessibility): translated by hand for accessibility, but **the English version remains the legally binding text**. The beta ribbon notes this. A native-speaker legal review is required to promote to "reviewed".
...
Zero recurring cost. Translations are static `.njk` files committed to the repo. Translations are written by hand, with native-speaker volunteers (board / membership) reviewing them. Only the GitHub Actions runner minutes count, which the public-repo allowance covers.
