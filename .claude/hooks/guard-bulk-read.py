...
A full read of archiveProgrammes.js or site.css puts thousands of lines into
the context and every following turn resends them. Targeted reads pass through
untouched, so the escape hatch is one flag away: grep for the region, then Read
with an offset and a limit. Threshold via CLAUDE_READ_MAX_LINES, default 800,
which catches 12 of the 130 tracked text files here.

Fail OPEN on anything unexpected. A guard that wrongly blocks normal work is
worse than the reads it prevents.

Kept byte-identical to the NetSec copy apart from this docstring, so a fix
ports across without a diff to read. See EISSeuropa/netsec.github.io#1794.
"""
import json
import os
import re
