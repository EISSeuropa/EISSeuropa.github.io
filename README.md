...
| **MINOR** (`1.x.0`) | A **big new project** — a new top-level page, a new automated pipeline, a new locale, a new top-level feature. The default for substantive work. | Adding a new language; new public page (`/2026`, `/404`); introducing the Indico sync; the live programme grid. |
...
- **Patch releases** ship the `### Index of changes` block only — no lede, no themed sections. People reading patch notes care about specifics, not narrative.
